...
class TimeSummaryHandler(Handler):
    ...
    def bind_signals(self):
        ...
        self.time_summary.connect(
            'data_update_end', self.render)
        GtkThread(
            target=self.time_summary.fetch_data,
            args=(self.week.get_selected_date(),),
            target_cb=gtk_thread_cb(lambda t: self.time_summary.emit('data_update_end'))
            ).start()

    def render(self, gobj=None):
        self.get_widget('labelTimeSummaryHoursToday').set_label(f'{self.time_summary.today:02.02f}')
        self.get_widget('labelTimeSummaryHoursYesterday').set_label(f'{self.time_summary.yesterday:02.02f}')
        self.get_widget('labelTimeSummaryHoursWeek').set_label(f'{self.time_summary.week:02.02f}')
        self.get_widget('labelTimeSummaryHoursMonth').set_label(f'{self.time_summary.month:02.02f}')
