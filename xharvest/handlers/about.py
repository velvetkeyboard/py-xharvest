...
class AboutHandler(Handler):
    ...
    def bind_data(self):
        ...
        self.builder.get_object('label_app_logo')\
            .set_from_file(get_img_path('xharvest.png'))
        self.builder.get_object('label_app_version')\
            .set_markup(f'v{__version__}')
        # -------------------------------------------[License and Registration]
        year_now = datetime.now().strftime('%Y')
        self.builder.get_object('label_app_license')\
            .set_markup(f'License - {__license__}')
        self.builder.get_object('label_app_year')\
            .set_markup(f'🄯 2020 - {year_now} {__app_name__}')
        # ---------------------------------------------------==[Author Section]
        ...
        self.builder.get_object('img_app_author_avatar')\
            .set_from_pixbuf(
                self.author.get_avatar_img_as_pixbuf())