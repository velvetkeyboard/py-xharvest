...
class CustomWidget():  # TODO cast to Factory pattern

    glade = ''
    root_widget = None

    def __init__(self, data, event_binds):
        super(Gtk.Widget, self).__init__()
        self.data = data
        self.event_binds = event_binds

        self.bind(builder)
        self.add(builder.get_object(self.root_widget))

    def bind(self, builder):
        raise NotImplemented


# def find_child_by_name(name, widget):
#     try:
#         if widget.get_children():
#             for w in widget.get_children():
#                 ret = find_child_by_name(name, w)
#                 if ret:
#                     return ret
#         else:
#             logger.info(f"{widget.get_name()} == {name}")
...
# def wipe_children(widget):
#     for c in widget.get_children():
#         print('child', c)
#         widget.remove(c)


class WeekDaysListBoxManager(object):

    def __init__(self, oauth2, box, lbx_timeentries):
        self.oauth2 = oauth2
        self.box = box
        self.lbx_timeentries = lbx_timeentries
        self.selected_date = datetime.now()  #date.today()

    def redraw(self):
        print('redraw', 'self.selected_date', self.selected_date)
        ...
        for i in range(start, end):
            ...
            print('WeekDayWidget', data)
            widget = WeekDayWidget(
                data,
                [
                    ('eventBoxWeekDay', 'button-press-event', self.date_selected),
                ],
            )
            self.box.add(widget)
        self.box.show_all()

    def date_selected(self, event_box, event_button):
        ...
        self.selected_date = datetime.strptime(date_iso, '%Y-%m-%dT%H:%M:%S.%f')
        self.lbx_timeentries.refresh(self.selected_date)


class TimeEntriesListBoxManager(object):
    def __init__(self, oauth2, lbx, spn, pop):
        '''
        Params:
            oauth2 (harvest.credentials.OAuth2Credential):
            lbx (Gtk.ListBox):
            spn (Gtk.Spinner):
            pop (Gtk.Popover):
        '''
    ...
    def add_time_entry(self, data):
        self.time_entries.append(data)
    ...
    @gtk_thread_class_cb
    def refresh_time_entries_cb(self, thread):
        for child in self.lbx.get_children():
            self.lbx.remove(child)
        for entry in self.time_entries:
            print('entry ->',entry['id'], entry['spent_date'], entry['notes'][:79])
            entry['popover'] = self.pop
            if self.selected_date.date() == \
                datetime.strptime(entry['spent_date'], '%Y-%m-%d').date():
                self.lbx.add(TimeEntryListBoxRow(entry))

        self.lbx.show_all()
        self.spn.hide()
        self.spn.stop()

    def refresh(self, selected_date=None, pull=None):
        if selected_date:
            self.selected_date = selected_date
        if pull or len(self.time_entries) == 0:
            self.spn.start()
            self.spn.show()
            CustomThread(
                target=self.refresh_time_entries,
                target_cb=self.refresh_time_entries_cb,
                ).start()
        else:
            self.refresh_time_entries_cb()
